...
function func.initUI()
	...
	do	--win
		...
		ui.gearWin:SetPoint("TOPLEFT", ui.open, "TOPRIGHT", 8, 28);
		...
		ui.scroll = alaScrollList.CreateScrollFrame(ui.gearWin, win_SizeX - 20, win_SizeY - 64, btn_SizeY, func.gm_CreateButton, func.gm_SetButton);
	...
	do	--secure quick panel & macro
		...
		function ui.secure:Update()
			...
			for i = 1, N do
				...
				if i == TOA then
					button:SetAttribute('macrotext', "/run func.takeoffAll()");
					-- button.info[1] = info[1];
					button:SetTitleAndIcon(L["Take_Off_All_ButtonText"], iconTable[37]);
				else
					local macroText = "/run func.equip(" .. setIndex .. ");\n";
