--## Author: Gello  ## Version: 1.4.2 Diy by y368413
...
function cql:ListEntryOnEnter()
   ...
   if isComplete and isComplete>0 then
			...
			--[[if isComplete or numObjectives == 0 then
				if not first and not oldcomplete[title] and numObjectives > 0 then
					...
					if isComplete == -1 then
						...
						if IsQuestWatched(index) then  --db.removeComplete and
							RemoveQuestWatch(index)
						end
					end
				end
      ...
      for i=1,numObjectives do
		--processObjective(questID, title, isTask, o, GetQuestLogLeaderBoard(o, index))  --above is new
