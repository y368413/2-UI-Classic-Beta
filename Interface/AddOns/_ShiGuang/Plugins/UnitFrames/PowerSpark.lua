...
for _, v in pairs({'PLAYER_ENTERING_WORLD','PLAYER_LEAVING_WORLD', 'UNIT_SPELLCAST_SUCCEEDED', 'UNIT_MAXPOWER', 'UNIT_POWER_FREQUENT'}) do
	PowerSparkFrame:RegisterEvent(v, 'player')
end
if select(2, UnitClass('player')) == 'ROGUE' then PowerSparkFrame:RegisterEvent('UNIT_AURA') end
PowerSparkFrame:SetScript('OnEvent', function(self, event, ...)
	if event == 'PLAYER_ENTERING_WORLD' then
		...
		function self.init(parent, key) --初始化
			...
			--if not ShiGuangDB then ShiGuangDB = {[key] = {}} end
			if not ShiGuangDB[key] then ShiGuangDB[key] = {} end
			...
			power.timer = ShiGuangDB[key].timer
		...
		if select(2, UnitClass('player')) == 'DRUID' and DruidBarFrame and DruidBarKey then self.init(DruidBarFrame, 'druid') end
	elseif event == 'PLAYER_LEAVING_WORLD' then
		ShiGuangDB.default.timer = self.default.timer
		if self.druid then ShiGuangDB.druid.timer = self.druid.timer end
