...
function S:TradeSkill_OnEvent(addon)
	...
	if skinIndex >= 2 then
		M:UnregisterEvent("ADDON_LOADED", S.TradeSkill_OnEvent)
	end
end

function S:TradeSkillSkin()
	if not MaoRUIPerDB["Skins"]["TradeSkills"] then return end

	M:RegisterEvent("ADDON_LOADED", S.TradeSkill_OnEvent)
end
