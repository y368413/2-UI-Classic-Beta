--## Author: Michael Röhrig  ## Version: .13.3 build 2
...
GameTooltip:HookScript("OnTooltipSetItem", function(self)
	...
	if(UnitAffectingCombat("player") and PlayerLevelIsHighEnoughToUseItem(itemId) and SearchTooltip(MakeRegexSafe(LOCKED)) and (SearchTooltip(string.gsub(LOCKED_WITH_SPELL, "%%s", ".*")) or SearchTooltip("Crochetage.*%(.*%).*requis")) and not SearchTooltip(ITEM_ENCHANT_DISCLAIMER)) then
		GameTooltip:AddLine(" ")
	...
	--]]
	if(not UnitAffectingCombat("player") and PlayerLevelIsHighEnoughToUseItem(itemId) and SearchTooltip(MakeRegexSafe(LOCKED)) and (SearchTooltip(string.gsub(LOCKED_WITH_SPELL, "%%s", ".*")) or SearchTooltip("Crochetage.*%(.*%).*requis")) and not SearchTooltip(ITEM_ENCHANT_DISCLAIMER)) then
